...
public class UINodeSingleton
{
    ...
    public List<Transform> AllFlagTrans(Transform go)
    {
        ...
        foreach (Transform item in childList)
        {
            ...
            if (temp == null)
            {
                Debug.LogError("无法从节点正确获取指定组件:" + string.Join("/",
                                   item.GetComponentsInParent<Transform>(true).Reverse().Select((s) =>
                                   {
                                       return s.name;
                                   }).ToArray()), item);
                continue;
    ...
    public UnityEngine.Object GetUINodeType(Transform trans, string keyname)
    {
        ...
        switch (type)
        {
            ...
            case "textmeshpro":
                ...
                break;
            case "textmeshproinput":
                obj = trans.GetComponent<TMP_InputField>();
                break;
            case "btn":
            ...
            case "depthlist":
                break;
            case "extendText":
                obj = trans.GetComponent<ExtendText>();
                break;
            case "layout":
                obj = trans.GetComponent<LayoutElement>();
                break;
            case "aroverlayout":
                break;
            case "arohorlayout":
            ...
            case "aorSlider":
                ...
                break;
            case "scrollrect":
                obj = trans.GetComponent<ScrollRect>();
                break;
            default:
                Debug.LogError(type);
                break;
