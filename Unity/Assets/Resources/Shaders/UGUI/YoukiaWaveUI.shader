Shader "YoukiaEngine/UI/YoukiaWaveUI"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half _MainTexSpeedX, _MainTexSpeedY;
            fixed _EdgeWidth,_MidOffset;
            fixed _ColorStrength;
            half4 _ColorTop, _MidColor;
            half4 _ButtomColor0, _ButtomColor1;
            half _FillAmount;
            half _Amplitude1, _Frequency1, _Speed1;
            half _Amplitude2, _Frequency2, _Speed2;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                half height = i.texcoord.y;
                half width = i.texcoord.x;

                half wave1 = _Amplitude1 * sin(width * _Frequency1 + _Speed1 * _Time.y);
                half wave2 = _Amplitude2 * sin(width * _Frequency2 + _Speed2 * _Time.y);
                half wave = wave1 + wave2;

                half f = _FillAmount + wave;
                ...
                {
                    half noise = tex2D(_NoiseTex, i.uv.zw + half2(_NoiseSpeedX, _NoiseSpeedY) * _Time.y).r;
                    noise *= _NoiseStrength;
                    fixed4 col = tex2D(_MainTex, i.uv.xy + half2(_MainTexSpeedX, _MainTexSpeedY) * _Time.y + noise); // 加上扰动UV后再采样主纹理
