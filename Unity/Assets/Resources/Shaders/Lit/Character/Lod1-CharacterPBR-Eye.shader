...
Shader "YoukiaEngine/Lit/Character/Lod1-CharacterPBR-Eye"
{
    Properties
    {
        [HDR]_Color("颜色", Color) = (1, 1, 1, 1)
		[NoScaleOffset]_MainTex("瞳孔纹理 RGB: 瞳孔颜色, Alpha: 瞳孔范围", 2D) = "white" {}
        [Header(Normal)]
        [NoScaleOffset] _BumpMetaMap("法线纹理, rg: normal, b: ao, a: 自发光", 2D) = "white" {}
        _NormalStrength ("法线强度矫正", Range(0, 2)) = 1
        _BumpScale ("瞳孔缩放", Range(0, 5)) = 1
        [Header(Diffuse)]
        ...
        Pass
        {
            ...
            CGPROGRAM
                ...
                FragmentOutput frag(v2f i)
                {
                    ...
                    half2 uvBump = _BumpScale * (i.uv.xy - 0.5) + 0.5;
                    fixed4 n = tex2D(_BumpMetaMap, uvBump);
                    
                    // normal
                    half3 normal = WorldNormal(n, i.TtoW);
                    normal = lerp(wsNormal, normal, c.a);
                    ...
                    half4 eyeColor = c;
                    half4 matcap = tex2D(_MatCapMap, (mul(UNITY_MATRIX_V, half4(normal, 0.0)).xyz * 0.5 + 0.5).xy) * _MatCapLighting;
                    eyeColor += matcap;
