...
public class GameEvent
{
    ...
    {
        List<GameEventHandler> handlerList;// = mEventDic.ContainsKey (type) ? mEventDic[type] : null;
        if (mEventDic.TryGetValue(type, out handlerList))
        {
            for (int i = handlerList.Count-1; i >=0 ; i--)
            {
                ...
                // 解决问题,如果在收到事件立马移除这个事件,会导致下一个事件出问题
                //if (i >= 0 && handlerList[i] != handler)
                //	i--;

                //                if (args.Length > 0 && args[0] is EventArgs && (args[0] as EventArgs).IsCancelDefaultAction)
                //                    break;
            }
        }
    }
