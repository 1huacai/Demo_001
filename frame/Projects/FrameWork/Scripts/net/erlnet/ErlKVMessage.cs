...
public class ErlKVMessage
{
    ...
    public override bool Equals(object obj)
    {
        ...
        ErlKVMessage _message = obj as ErlKVMessage;
        if(null == _message )
        {
            return false;
        }

        if( _message.Cmd == Cmd && _message.Key.Count == Key.Count && _message.Value.Count == Value.Count)
        {
            for( int i = 0; i<Key.Count;++i)
            {
                if( Key [i] != _message.Key [i])
                {
                    return false;
                }
                if (Value[i] != _message.Value[i])
                {
                    return false;
                }
            }
