...
namespace FrameWork.Graphics
{
    ...
    public class LightManager : MonoBehaviour
    {
        ...
        public LightInfo DirectionalLight
        {
            ...
            get
            {
                if (Application.isPlaying && SingletonManager.GetManager<GraphicsManager>() != null && SingletonManager.GetManager<GraphicsManager>().isInited)
                    return SingletonManager.GetManager<GraphicsManager>().SunLight;
                else
                {
        ...
        {
            if (light.GetComponent<Light>().type == LightType.Directional)
            {
                return;
            }
        ...
        private void UpdateShader()
        {
            ...
            {
                if (Info == null || Info.gameObject == null || Info.gameObject.activeInHierarchy == false || Info.gameObject.activeSelf == false || Info.GetComponent<Light>().intensity <= 0)
                    continue;
            ...
            else
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    Light lt = lists[i].GetComponent<Light>();
                    Shader.SetGlobalVector("_CustomLightColor" + i, lt.color * lt.intensity);
                ...
                if (DirectionalLight != null)
                {
                    ...
                    Light lt = DirectionalLight.GetComponent<Light>();
                    rot.w = lt.intensity;
