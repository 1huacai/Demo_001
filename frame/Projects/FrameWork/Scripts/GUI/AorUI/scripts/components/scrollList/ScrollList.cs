...
using FrameWork.App;
using FrameWork.GUI.AorUI.Components.Assets;
using FrameWork.GUI.AorUI.Core;
using FrameWork.GUI.AorUI.events;
using CoreFrameWork;
...
namespace FrameWork.GUI.AorUI.Components
{
    ...
    public class ScrollList : AorUIComponent
    {
        ...
        private void UpdataItemSize(int index, ListItem li, int InputIndex, float size_y, bool add)
        {
            ...
            {//暂不写横排集
             // li.gameObject.GetComponent<RectTransform>().anchoredPosition = new Vector2(index * (_ItemSize.x + _padding), li.gameObject.GetComponent<RectTransform>().anchoredPosition.y);
                li.gameObject.GetComponent<RectTransform>().anchoredPosition = new Vector2((index * (_ItemSize.x + _padding)) + item_y, li.gameObject.GetComponent<RectTransform>().anchoredPosition.y);
                Log.Warning("index:"+ index+" "+ (index * (_ItemSize.x + _padding)) + item_y);
            }
