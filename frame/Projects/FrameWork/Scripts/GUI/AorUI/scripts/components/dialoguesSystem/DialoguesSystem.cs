    ...
    public class DialoguesSystem : AorUIComponent {
        ...
        public override void OnAwake() {
            ...
            if (TitleBox != null) {
                ...
                Log.Error("DialoguesSystem.OnAwake Error :: 缺少重要模块 : TitleBox#");
                return;
            }
            ...
            if (DialogBox != null) {
              ...
              //  _textTyper.FinishCall(_textTyper.GetType().ToString());
                ...
                Log.Error("DialoguesSystem.OnAwake Error :: 缺少重要模块 : DialogBox#");
                return;
            }
        ...
        protected override void DrawUI() {
            ...
            foreach (DialogPortraitInfo info in _data.PortraitDic.Values) {
                ...
                if (info.PortraitType == DialogPortraitType.prefab) {
                    //YKApplication.Instance.GetManager<ResourcesManager>().PoolMg.CacheObjToPool(info.PortraitPath, (obj) => { });
                    AorUIAssetLoader.CachePrefab(info.PortraitPath, (obj) => { });

                } else {
                    //YKApplication.Instance.GetManager<ResourcesManager>().CacheObject(info.PortraitPath, (obj) => { });
                ...
                if (info.PortraitTitleType == DialogPortraitTitleType.Image) {
                    //YKApplication.Instance.GetManager<ResourcesManager>().CacheObject(info.PortraitTitlePath, (obj) => { });
        ...
        public void Play(int index = 0) {
            ...
            if (currentInfo.PortraitType == DialogPortraitType.prefab) {
                ...
                if (imgPaths.Length < 2) {
                    Log.Error("DialoguesSystem.Play Error : 肖像数据有错误, 数据中不包含正确的Sprite名称: >> 错误的DialogPortraitInfo[" + currentInfo.PortraitID + "]");
                    return;
                }
                ...
                AorUIAssetLoader.LoadUIAssetAsync(currentInfo.PortraitPath, (texAsset,obj) => {
                    ...
                    FResourceRef resRef = obj[0] as FResourceRef;
            ...
            if (currentInfo.PortraitTitleType == DialogPortraitTitleType.Text) {
                ...
                string[] imgPaths = currentInfo.PortraitPath.Split('@');
                if (imgPaths.Length < 2) {
                    Log.Error("DialoguesSystem.Play Error : 肖像标题数据有错误, 数据中不包含正确的TitleSprite名称: >> 错误的DialogPortraitInfo[" + currentInfo.PortraitID + "]");
                    return;
                }
                //Image
                AorUIAssetLoader.LoadSprite(imgPaths[0], imgPaths[1], (sp,objRef) => {
