...
namespace FrameWork.GUI.AorUI.effects
{
    ...
    public class Gradient : Outline, IGrayMember
    {
        ...
        public void SetKeys(GradientColorKey[] colorKeys, GradientAlphaKey[] alphaKeys = null)
        {
            if (alphaKeys == null)
            {
                alphaKeys = GetGradient().alphaKeys;
            }
            GetGradient().SetKeys(colorKeys, alphaKeys);
                ...
                case Type.Vertical:
                    {
                        ...
                        for (int i = 0; i < _vertexList.Count; i++)
                        {
                            ...
                            vertex.color = BlendColor(vertex.color,
                                GradientColor.Evaluate((vertex.position.y - bottom) * height - Offset));
                            _vertexList[i] = vertex;
                            // helper.SetUIVertex(vertex, i);
                        }
                        helper.AddUIVertexTriangleStream(_vertexList);
